...
test.describe('File Operations', () => {
  ...
  test.describe('File Dialog Operations', () => {
    test('should show open dialog with correct filters', async ({
      electronPage,
      electronMain,
    }) => {
      ...
      dialogOptions = await evaluateInMain(
        electronMain,
        () => (global as any).__capturedDialogOptions
      )
    ...
    test('should show save dialog with correct filters', async ({
      electronPage,
      electronMain,
    }) => {
      let dialogOptions: any = null
      ...
      dialogOptions = await evaluateInMain(
        electronMain,
        () => (global as any).__capturedSaveDialogOptions
      )
    ...
    test('should handle successful file selection', async ({
      electronPage,
      electronMain,
    }) => {
    ...
    test('should handle menu-triggered file open', async ({
      electronPage,
      electronMain,
    }) => {
      // Set up listener for file open message
      const fileOpenPromise = waitForIPCMessage(
        electronPage,
        'menu-open-file',
        5000
      )
  ...
  test.describe('Notes Persistence', () => {
    test('should save and load notes correctly', async ({ electronPage }) => {
      ...
      // Save notes
      const saveResult = await sendIPCMessage(
        electronPage,
        'saveNotes',
        testNotes
      )
      ...
      const specialNotes = [
        {
          ...
          content:
            'Content with special chars: àáâãäåæçèéêëìíîïñòóôõöøùúûüý\nNew line\tTab\r\nWindows line ending',
          tags: ['测试', 'tëst', '🏷️'],
        },
      ]
    ...
    test('should load empty array when notes file does not exist', async ({
      electronPage,
      electronMain,
    }) => {
      ...
      await evaluateInMain(electronMain, async ({ app }) => {
        ...
        (global as any).__originalNotesPath = join(
    ...
    test('should verify notes file location', async ({
      electronPage,
      electronMain,
    }) => {
      ...
      await evaluateInMain(electronMain, async ({ ipcMain, app }) => {
        const { existsSync, unlinkSync, writeFileSync, readFileSync } =
          await import('fs')
        const { join } = await import('path')
  ...
  test.describe('Export Operations', () => {
    test('should handle export IPC functionality', async ({ electronPage }) => {
      ...
      await electronPage.evaluate(() => {
        if (window.electronAPI) {
          ...
          window.electronAPI.exportNote = async (
            noteData: any,
            format: string
          ) => {
            return {
              success: true,
              filePath: `/test/path/${noteData.filename || 'test'}.${format}`,
            }
          }
    ...
    test('should trigger PDF export menu action', async ({
      electronPage,
      electronMain,
    }) => {
      ...
      await page.evaluate(() => {
        ...
        console.log(
          '[Test] Checking onMessage function:',
      ...
      const menuDebugInfo = await electronMain.evaluate(
        ({ Menu, BrowserWindow }) => {
          const menu = Menu.getApplicationMenu()
          ...
          const exportItem = fileMenu?.submenu?.items.find(
            (item: any) => item.label === 'Export...'
          )
          const pdfItem = exportItem?.submenu?.items.find(
            (item: any) => item.label === 'Export as PDF'
          )
          ...
          return {
            ...
            hasPdfItem: !!pdfItem,
            pdfItemClickable: !!(
              pdfItem && typeof pdfItem.click === 'function'
            ),
    ...
    test('should trigger HTML export menu action', async ({
      electronPage,
      electronMain,
    }) => {
      const page = electronPage
      ...
      await electronMain.evaluate(({ Menu }) => {
        ...
        const htmlItem = exportItem?.submenu?.items.find(
          (item: any) => item.label === 'Export as HTML'
        )
        htmlItem?.click()
      })
    ...
    test('should trigger Markdown export menu action', async ({
      electronPage,
      electronMain,
    }) => {
      const page = electronPage
      ...
      // Set up IPC listener before triggering menu action
      const exportPromise = waitForIPCMessage(
        page,
      ...
      await electronMain.evaluate(({ Menu }) => {
        ...
        const markdownItem = exportItem?.submenu?.items.find(
          (item: any) => item.label === 'Export as Markdown'
        )
        markdownItem?.click()
      })
  ...
  test.describe('Menu File Operations', () => {
    test('should trigger New Note menu action', async ({
      electronPage,
      electronMain,
    }) => {
      const page = electronPage
      ...
      await electronMain.evaluate(({ Menu }) => {
        const menu = Menu.getApplicationMenu()
        const fileMenu = menu?.items.find((item: any) => item.label === 'File')
        const newNoteItem = fileMenu?.submenu?.items.find(
          (item: any) => item.label === 'New Note'
        )
    ...
    test('should trigger Save menu action', async ({
      electronPage,
      electronMain,
    }) => {
      const page = electronPage
    ...
    test('should trigger Save As menu action', async ({
      electronPage,
      electronMain,
    }) => {
      const page = electronPage
    ...
    test('should have correct keyboard shortcuts', async ({ electronMain }) => {
      ...
      const shortcutMap = new Map(
        shortcuts?.map((item) => [item.label, item.accelerator])
      )
  ...
  test.describe('File System Edge Cases', () => {
    ...
    test('should handle malformed notes file', async ({
      electronPage,
      electronMain,
    }) => {
    ...
    test('should handle notes with binary-like content', async ({
      electronPage,
    }) => {
