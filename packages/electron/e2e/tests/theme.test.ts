...
test.describe('Theme System', () => {
  ...
  test.describe('Theme IPC Handlers', () => {
    test('should get current theme', async ({ electronPage }) => {
      const currentTheme = await sendIPCMessage<string>(
        electronPage,
        'getTheme'
      )
    ...
    test('should set theme to light', async ({
      ...
      const result = await sendIPCMessage<string>(
        electronPage,
        'setTheme',
        'light'
      )
      ...
      const mainTheme = await evaluateInMain(
        electronMain,
      ...
      const getThemeResult = await sendIPCMessage<string>(
        electronPage,
        'getTheme'
      )
    ...
    test('should handle all valid theme values in sequence', async ({
      electronPage,
      electronMain,
    }) => {
      ...
      for (const theme of themes) {
        const setResult = await sendIPCMessage<string>(
          electronPage,
          'setTheme',
          theme
        )
        ...
        const mainResult = await evaluateInMain(
          electronMain,
          ({ nativeTheme }) => nativeTheme.themeSource
        )
  ...
  test.describe('Menu Theme Integration', () => {
    test('should have theme submenu with correct structure', async ({
      electronMain,
    }) => {
      const themeMenuStructure = await evaluateInMain(
        electronMain,
        ({ Menu, nativeTheme }) => {
          const menu = Menu.getApplicationMenu()
          const viewMenu = menu?.items.find((item) => item.label === 'View')
          const themeItem = viewMenu?.submenu?.items.find(
    ...
    test('should update menu when theme changes via IPC', async ({
      electronPage,
      electronMain,
    }) => {
      const initialTheme = await sendIPCMessage<string>(
        electronPage,
        'getTheme'
      )
      ...
      // Check menu state
      const menuState = await evaluateInMain(
        electronMain,
        ({ Menu, nativeTheme }) => {
          const menu = Menu.getApplicationMenu()
          const viewMenu = menu?.items.find((item) => item.label === 'View')
          ...
          return {
            currentTheme: nativeTheme.themeSource,
            lightChecked: themeItem?.submenu?.items.find(
              (item) => item.label === 'Light'
            )?.checked,
            darkChecked: themeItem?.submenu?.items.find(
              (item) => item.label === 'Dark'
            )?.checked,
            systemChecked: themeItem?.submenu?.items.find(
    ...
    test('should change theme when menu item is clicked', async ({
      electronPage,
      electronMain,
    }) => {
      const initialTheme = await sendIPCMessage<string>(
      ...
      const themeChangePromise = waitForIPCMessage(
        electronPage,
        'theme-changed',
        5000
      )
      ...
      await evaluateInMain(
        ...
        ({ Menu }, targetTheme) => {
          ...
          const targetItem = themeItem?.submenu?.items.find(
            (item) => item.label?.toLowerCase() === targetTheme
          )
          targetItem?.click()
        },
        targetTheme
    ...
    test('should handle all theme menu items', async ({
      electronPage,
      electronMain,
    }) => {
      ...
      for (const theme of themes) {
        ...
        await evaluateInMain(
          ...
          ({ Menu }, theme) => {
            ...
            const targetItem = themeItem?.submenu?.items.find(
              (item) => item.label?.toLowerCase() === theme
            )
            targetItem?.click()
          },
          theme
  ...
  test.describe('Native Theme Events', () => {
    test('should handle native theme updated events', async ({
      electronPage,
      electronMain,
    }) => {
      ...
      const nativeThemeUpdatePromise = waitForIPCMessage(
        page,
    ...
    test('should reflect system theme when set to system', async ({
      electronPage,
      electronMain,
    }) => {
      // Set theme to system
      ...
      expect(typeof systemThemeInfo.shouldUseInvertedColorScheme).toBe(
        'boolean'
      )
    })

    test('should emit native theme updated on system changes', async ({
      electronPage,
      electronMain,
    }) => {
      ...
      // Set up listener
      const updatePromise = waitForIPCMessage(
        electronPage,
        'native-theme-updated',
        5000
      )
  ...
  test.describe('Theme Message Broadcasting', () => {
    test('should send theme-changed message when theme is set', async ({
      electronPage,
      electronMain,
    }) => {
      const initialTheme = await sendIPCMessage<string>(
    ...
    test('should broadcast theme changes to all windows', async ({
      electronPage,
      electronMain,
    }) => {
      ...
      const windowCount = await evaluateInMain(
        electronMain,
        ({ BrowserWindow }) => {
      ...
      const messagesSent = await evaluateInMain(
        electronMain,
        ({ BrowserWindow, Menu }, targetTheme) => {
          let sentCount = 0
  ...
  test.describe('Theme Persistence and Consistency', () => {
    test('should maintain theme consistency across operations', async ({
      electronPage,
      electronMain,
    }) => {
      ...
      const themeAfterOps = await sendIPCMessage<string>(
        electronPage,
        'getTheme'
      )
    ...
    test('should handle theme during window lifecycle', async ({
      electronPage,
    }) => {
      // Set initial theme
    ...
    test('should synchronize theme across multiple windows', async ({
      electronPage,
      electronMain,
    }) => {
      // Create additional window
      ...
      const allWindowsTheme = await evaluateInMain(
        electronMain,
        ({ nativeTheme }) => {
  ...
  test.describe('Theme Edge Cases', () => {
    ...
    test('should handle theme switching during heavy operations', async ({
      electronPage,
    }) => {
    ...
    test('should handle multiple rapid theme menu clicks', async ({
      electronPage,
      electronMain,
    }) => {
