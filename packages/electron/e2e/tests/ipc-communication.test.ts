...
test.describe('IPC Communication', () => {
  ...
  test.describe('File Operations IPC', () => {
    ...
    test('should handle save-notes error gracefully', async ({
      electronPage,
    }) => {
    ...
    test('should handle show-open-dialog IPC call', async ({
      electronPage,
      electronMain,
    }) => {
    ...
    test('should handle show-save-dialog IPC call', async ({
      electronPage,
      electronMain,
    }) => {
      // Mock the dialog to avoid actually showing it
    ...
    test('should handle dialogs when no main window exists', async ({
      electronPage,
      electronMain,
    }) => {
  ...
  test.describe('Window Operations IPC', () => {
    test('should handle create-window IPC call', async ({
      electronPage,
      electronMain,
    }) => {
    ...
    test('should handle window state operations', async ({
      electronPage,
      electronMain,
    }) => {
      // Test is-maximized
      const initialMaximized = await sendIPCMessage<boolean>(
        electronPage,
        'isMaximized'
      )
      ...
      const afterToggleMaximized = await sendIPCMessage<boolean>(
        electronPage,
        'isMaximized'
      )
      ...
      const isMinimized = await evaluateInMain(
        electronMain,
        ({ BrowserWindow }) => {
    ...
    test('should handle close-window IPC call', async ({
      electronPage,
      electronMain,
    }) => {
    ...
    test('should handle window operations when no focused window exists', async ({
      electronPage,
      electronMain,
    }) => {
      ...
      await expect(
        sendIPCMessage(electronPage, 'minimizeWindow')
      ).resolves.toBeUndefined()
      await expect(
        sendIPCMessage(electronPage, 'maximizeWindow')
      ).resolves.toBeUndefined()
      await expect(
        sendIPCMessage(electronPage, 'closeWindow')
      ).resolves.toBeUndefined()

      const isMaximized = await sendIPCMessage<boolean>(
        electronPage,
        'isMaximized'
      )
  ...
  test.describe('Theme Operations IPC', () => {
    ...
    test('should handle set-theme IPC call', async ({
      electronPage,
      electronMain,
    }) => {
      const originalTheme = await sendIPCMessage<string>(
        electronPage,
        'getTheme'
      )
      ...
      const mainProcessTheme = await evaluateInMain(
        electronMain,
    ...
    test('should handle all valid theme values', async ({ electronPage }) => {
      ...
      for (const theme of themes) {
        ...
        const getCurrentTheme = await sendIPCMessage<string>(
          electronPage,
          'getTheme'
        )
  ...
  test.describe('Utility Operations IPC', () => {
    test('should handle show-notification IPC call', async ({
      electronPage,
    }) => {
      ...
      await expect(
        sendIPCMessage(electronPage, 'showNotification', title, body)
      ).resolves.toBeUndefined()
    })

    test('should handle open-dev-tools IPC call', async ({
      electronPage,
      electronMain,
    }) => {
    ...
    test('should handle reload-window IPC call', async ({
      electronPage,
      electronMain,
    }) => {
  ...
  test.describe('IPC Message Broadcasting', () => {
    test('should receive native-theme-updated messages', async ({
      electronPage,
      electronMain,
    }) => {
      ...
      const themeUpdatePromise = waitForIPCMessage(
        page,
    ...
    test('should receive theme-changed messages when theme is set', async ({
      electronPage,
      electronMain,
    }) => {
      const originalTheme = await sendIPCMessage<string>(
    ...
    test('should handle IPC communication error scenarios', async ({
      electronPage,
    }) => {
  ...
  test.describe('File System Integration', () => {
    test('should persist and load notes correctly', async ({
      electronPage,
    }) => {
      // Create test data
      const testNotes = [
