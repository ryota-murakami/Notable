...
import { getAllFixtures, SEED_QUERIES, createTestSession } from '../utils/db-fixtures'

const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL || 'http://localhost:54321'
const supabaseServiceKey = process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_ANON_KEY

if (!supabaseServiceKey) {
  console.error('❌ Missing Supabase service key. Set SUPABASE_SERVICE_ROLE_KEY or SUPABASE_ANON_KEY')
  process.exit(1)
}
...
async function resetDatabase() {
  try {
    ...
    await supabase.from('note_tags').delete().in('note_id', ['test-note-1', 'test-note-2', 'test-note-3'])
    await supabase.from('notes').delete().in('user_id', ['test-user-1', 'test-user-2'])
    await supabase.from('tags').delete().in('user_id', ['test-user-1', 'test-user-2'])
    await supabase.from('folders').delete().in('user_id', ['test-user-1', 'test-user-2'])
    ...
    const { error: foldersError } = await supabase
      .from('folders')
      .insert(fixtures.folders.map(folder => ({
        id: folder.id,
        name: folder.name,
    ...
    const { error: notesError } = await supabase
      .from('notes')
      .insert(fixtures.notes.map(note => ({
        id: note.id,
        title: note.title,
    ...
    const { error: noteTagsError } = await supabase
      .from('note_tags')
      .insert(fixtures.note_tags.map(nt => ({
        id: nt.id,
        note_id: nt.note_id,
        tag_id: nt.tag_id,
    ...
    if (noteTagsError) {
      ...
      console.info(`✅ Inserted ${fixtures.note_tags.length} note-tag relationships`)
    }
    ...
    console.info(`   • Note-Tags: ${fixtures.note_tags.length} relationships inserted`)
...
async function checkEnvironment() {
  ...
  if (!supabaseUrl.includes('localhost') && !supabaseUrl.includes('127.0.0.1')) {
    console.warn('⚠️  Warning: Not using local Supabase. Current URL:', supabaseUrl)
    ...
    if (!process.env.CI && !process.env.FORCE_RESET) {
      console.error('❌ Refusing to reset non-local database without FORCE_RESET=true')
      process.exit(1)
    }
  }
