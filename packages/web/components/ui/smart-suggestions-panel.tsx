...
export function SmartSuggestionsPanel({
  ...
  useEffect(() => {
    fetchSuggestions()
  }, [context, noteId, currentContent, lastRefresh])
  ...
  return (
    <Card className={cn("w-80 shadow-lg border-l-4 border-l-blue-500", className)}>
            ...
            <div className="space-y-4">
              {Object.entries(groupedSuggestions).map(([type, typeSuggestions]) => (
                <div key={type} className="space-y-2">
                  ...
                  {Object.keys(groupedSuggestions).indexOf(type) < Object.keys(groupedSuggestions).length - 1 && (
                    <Separator />
                  )}
                </div>
              ))}
...
export function useSmartSuggestions({
  ...
  }, [context, noteId, currentContent, enabled])
  ...
  return (
    <div className="flex items-center gap-2 p-2 bg-blue-50 border border-blue-200 rounded-lg text-sm">
      ...
      <div className="flex gap-1">
        <Button size="sm" variant="outline" onClick={onAccept} className="h-6 px-2">
          Accept
        </Button>
