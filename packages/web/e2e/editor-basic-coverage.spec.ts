...
test.describe('Editor Basic Coverage', () => {
  test('should debug template picker behavior', async ({ page }) => {
    ...
    const templatePickerExists = await page
      ...
      .count()
    console.log(`🔍 Template picker dialogs found: ${templatePickerExists}`)

    // Check for any dialogs
    const dialogs = await page.locator('[role="dialog"]').count()
    console.log(`🔍 Total dialogs found: ${dialogs}`)

    // Check network requests to template APIs
    const templateRequests = consoleMessages.filter((msg) =>
      msg.includes('/api/templates')
    )
    console.log(`🌐 Template API requests: ${templateRequests.length}`)
    templateRequests.forEach((req) => console.log(`   ${req}`))
    ...
    if (templatePickerExists > 0) {
      console.log('✅ Template picker is working!')
      ...
      if (await blankNoteButton.isVisible()) {
        console.log('🖱️  Clicking Blank Note button...')
        await blankNoteButton.click()
        await page.waitForTimeout(3000)
        const currentUrl = page.url()
        console.log(`🔗 Current URL after blank note: ${currentUrl}`)

        // Check if an editor appeared
        const editorVisible = await page
          .locator('[contenteditable="true"]')
          .isVisible()
        console.log(`✏️  Editor visible after blank note: ${editorVisible}`)
      }
    } else {
      console.log(
