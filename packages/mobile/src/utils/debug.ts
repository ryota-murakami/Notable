...
export class PerfMonitor {
  ...
  static end(label: string) {
    ...
    DevLogger.log(`Performance: ${label} took ${duration.toFixed(2)}ms`, color)
  ...
  static init() {
    ...
    global.fetch = async (input: any, init?: any) => {
      ...
      try {
        ...
        const duration = Date.now() - startTime
        DevLogger.error(
          `🌐 [${id}] Network error (${duration.toFixed(0)}ms)`,
          error as Error
        )
        this.requests.delete(id)
        throw error
...
export function initializeDebugTools() {
  ...
  DevLogger.success(
    'Debug tools available globally: DevLogger, PerfMonitor, MemoryMonitor, NetworkDebugger'
  )
