...
class MobileDevManager {
  ...
  async startMetroBundler() {
    ...
    return new Promise((resolve) => {
      ...
      metro.stdout.on('data', (data) => {
        ...
        if (
          ...
          process.stdout.write(
            `  ${colors.dim}${output.trim()}${colors.reset}\n`
          )
        }
      ...
      setTimeout(() => {
        if (!metroReady) {
          this.log(
            '  ⚠️  Metro bundler is taking longer than expected...',
            'yellow'
          )
          resolve(metro)
        }
  ...
  async showDevMenu() {
    ...
    const menuItems = [
      {
        key: '1',
        label: 'Start iOS Development',
        icon: '📱',
        available: this.platform === 'darwin',
      },
      {
        key: '2',
        label: 'Start Android Development',
        icon: '🤖',
        available: true,
      },
      {
        key: '3',
        label: 'Start Both Platforms',
        icon: '🚀',
        available: this.platform === 'darwin',
      },
      ...
      {
        key: '4',
        label: 'Setup iOS Simulator',
        icon: '📲',
        available: this.platform === 'darwin',
      },
      {
        key: '5',
        label: 'Setup Android Emulator',
        icon: '📲',
        available: true,
      },
      {
        key: '6',
        label: 'Reset Simulators/Emulators',
        icon: '🔄',
        available: true,
      },
      { key: '', label: '', icon: '', available: true },
    ...
    menuItems.forEach((item) => {
      if (!item.label) {
        ...
        this.log(
          `  ${item.icon}  [${item.key}] ${item.label} (unavailable)`,
          'dim'
        )
      }
    })
  ...
  async runE2ETests() {
    ...
    const platform = await new Promise((resolve) => {
      rl.question(
        `${colors.yellow}Select platform (ios/android/both): ${colors.reset}`,
        (answer) => {
          resolve(answer.toLowerCase())
        }
      )
    ...
    if (command) {
      switch (command) {
        ...
        default:
          ...
          this.log(
            'Usage: pnpm mobile:dev [ios|android|reset|clean|debug]',
            'yellow'
          )
          return
      }
...
if (process.platform !== 'win32') {
  ['ios-simulator.js', 'android-emulator.js', 'mobile-dev.js'].forEach(
    (script) => {
