...
class AndroidEmulatorManager {
  ...
  async checkRequirements() {
    ...
    // Check ANDROID_HOME
    if (!this.androidHome) {
      issues.push('ANDROID_HOME or ANDROID_SDK_ROOT environment variable is not set');
    }
    ...
    if (issues.length > 0) {
      ...
      this.log('\nPlease install Android Studio and set up the Android SDK:', 'yellow');
  ...
  async createAVD() {
    try {
      ...
      const avdmanager = path.join(this.androidHome, 'cmdline-tools', 'latest', 'bin', 'avdmanager');
  ...
  async startEmulator(avdName) {
    try {
      ...
      exec(`${emulatorPath} -avd ${avdName} -no-snapshot-load -no-audio -no-boot-anim`, 
        (error) => {
  ...
  async installExpoGo() {
    try {
      ...
      const { stdout } = await execAsync('adb shell pm list packages | grep host.exp.exponent || true');
      ...
      const expoGoUrl = 'https://d1ahtucjixef4r.cloudfront.net/Exponent-2.32.11.apk';
      ...
      this.log('You can install it manually from the Play Store in the emulator.', 'cyan');
    }
  }

  async setupEmulator() {
    this.log('🚀 Setting up Android Emulator for Notable development...', 'bright');
    ...
    if (runningEmulators.length > 0) {
      this.log(`✅ Using already running emulator: ${runningEmulators[0]}`, 'green');
      return;
    }
  ...
  async resetEmulator() {
    try {
      ...
      if (avds.includes(this.emulatorName)) {
        ...
        const emulatorPath = path.join(this.androidHome, 'emulator', 'emulator');
        await execAsync(`${emulatorPath} -avd ${this.emulatorName} -no-snapshot-save -no-snapshot-load -wipe-data`);
      }
  ...
  try {
    switch (command) {
      ...
      default:
        ...
        manager.log('  list          - List available AVDs and running emulators');
