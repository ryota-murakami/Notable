...
function setupDevEnvironment() {
  ...
  if (majorVersion < 18) {
    log(
      `❌ Node.js ${nodeVersion} is too old. Please install Node.js 18 or later.`,
      'red'
    )
    process.exit(1)
  }
  ...
  if (!fs.existsSync(envPath) && fs.existsSync(envExamplePath)) {
    ...
    log(
      '⚠️  No .env.example found, please create .env.local manually',
      'yellow'
    )
  }
  ...
  if (!androidHome) {
    ...
    if (fs.existsSync(sdkmanager)) {
      ...
      try {
        ...
        log(
          '⚠️  Some Android components may need manual installation',
          'yellow'
        )
      }
    }
  ...
  const tools = [
    ...
    },
    {
      ...
      check: 'which react-native-debugger',
