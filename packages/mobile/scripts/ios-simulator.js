...
class IOSSimulatorManager {
  ...
  async findPreferredSimulator() {
    ...
    if (!simulator) {
      simulator = devices.find(device => 
        device.name.includes('iPhone') && device.state !== 'Booted'
      );
    }
  ...
  async openSimulator(udid) {
    try {
      ...
      this.log(`Warning: Could not open Simulator app: ${error.message}`, 'yellow');
    }
  }
  ...
  async setupSimulator() {
    ...
    let bootedSim = await this.findBootedSimulator();
    if (bootedSim) {
  ...
  async installExpoGo(udid) {
    try {
      ...
      this.log(`Warning: Could not verify Expo Go installation: ${error.message}`, 'yellow');
    ...
    switch (command) {
      ...
      case 'list':
        ...
        devices.forEach(device => {
          ...
          manager.log(`  ${status} ${device.name} (${device.runtime})`, 
